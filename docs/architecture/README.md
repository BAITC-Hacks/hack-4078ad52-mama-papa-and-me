...
Входные схемы находятся в [contracts](../../src/contracts/index.ts). Клиент передаёт IDs мер/районов и версии, сервер сам определяет цены, эффекты и Score. Ошибки имеют `{error, details?}` и HTTP 400/403/404/413/415/429/500. Для preview неполного набора score/delta равны null. Серверная проверка Origin/Host/JSON защищает локальные операции записи от обычных межсайтовых запросов; это не система аккаунтов или публичный API.
